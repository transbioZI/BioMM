...
omics2genelist <- function(data, featureAnno, restrictUp = 500, 
    restrictDown = 5) {
    ...
    names(matlist) <- genes
    ## exclude the label
    geneSize <- unlist(lapply(matlist, function(i) {
        ncol(i) - 1
    }))
...
omics2chrlist <- function(data, probeAnno) {
    ...
    probeAnno <- probeAnno[is.element(probeAnno[, "ID"], probeName), c("ID", 
        "chr")]
    if (nrow(probeAnno) == 0) {
    ...
    ## exclude the label
    probeNperChr <- unlist(lapply(matlist, function(i) {
        ncol(i) - 1
    }))
