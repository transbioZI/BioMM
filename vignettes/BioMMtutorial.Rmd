...
featureAnno=NULL 
set.seed(1)
trainIndex <- sample(nrow(expData), nrow(expData)*0.5)
trainData <- expData[trainIndex,]
testData <- expData[-trainIndex,]
trainDataY <- trainData[,1]
testDataY <- testData[,1]
table(trainDataY)
table(testDataY)
...
set.seed(123)
result <- BioMM(trainData=trainData, testData=testData, 
                pathlistDB=kegglistSub[1:7], featureAnno, 
                restrictUp=200, restrictDown=10, minPathSize=10, 
                supervisedStage1, typePCA="regular", 
                resample1="BS", dataMode="allTrain",
                repeatA1=50, repeatA2=1, repeatB1=10, repeatB2=1, 
...
``` {r topPathFeatures, eval=F, fig.show="hold"} 
...
``` {r cirPlot, eval=F, fig.show="hold"}  
core <- MulticoreParam(workers = 10)   
